...
class CognitiveAgent:
    ...
    def observe(self, env: str, task: str):
        """
        Observe current context and possibly revise goal
        """
        if self.goal_policy.enabled and self.step % self.goal_policy.interval == 0:
            new_goal = self.goal_policy.maybe_update_goal(env, task)
            if new_goal:
                self.goal = new_goal

    def update(self, env: str, task: str, action: str, success: bool):
        """
        Store experience and update memories
        """
        ep = Episode(
            step=self.step,
            goal=self.goal,
